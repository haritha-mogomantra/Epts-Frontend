...
const handleDelete = async (empId) => {
  if (!window.confirm("Are you want to delete this employee?"))return;
  setLoading(true);
  try{
  ...
  const requiredFields = ["first_name", "email", "role", "department_code", "joining_date"];
  ...
  if (missingFields.length > 0) {
    ...
    setLoading(false);
    setAlert({
      message: `⚠️ Please fill all mandatory fields before saving: ${missingFields.join(", ")}`,
      type: "warning",
    });
    return;
  }
                  ...
                  <tr key={emp.id}>
                    ...
                    <td className="text-nowrap">
                      <div className="d-flex justify-content-end align-items-center gap-2">
                        <button className="btn btn-sm btn-outline-primary" title="Edit" onClick={()=> handleEdit(emp)}>
                          <i className="bi bi-pencil-square"></i>
 
                        </button>
                        <button className="btn btn-sm btn-outline-success" title="View" onClick={()=>handleView(emp)}>
                          <i className="bi bi-eye"></i>
 
                        </button>
                        <button className="btn btn-sm btn-outline-danger" title="Delete" onClick={()=>handleDelete(emp.emp_id)}>
                          <i className="bi"></i>
